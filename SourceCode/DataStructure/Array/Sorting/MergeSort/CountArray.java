package DataStructure.Array.Sorting.MergeSort;
...
public class CountArray {
    public int[] countArray(int[] array) {
        ...
        int[] indexHelper = new int[array.length]; // store unsorted index that left is sorted and right is sorted
    ...
    private void merge(int[] array, int[] indexArray, int[] countArray, int[] indexHelper, int left, int mid, int right) {
        ...
        int cur = left;
        while (l <= mid) {
            if (r > right || array[indexHelper[l]] <= array[indexHelper[r]]) {
